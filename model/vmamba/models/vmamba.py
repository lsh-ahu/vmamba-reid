...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.patch_embed(x) # ([64, 3, 256, 128]) => ([64, 64, 32, 128])
        for layer in self.layers: # ([64, 64, 32, 128]) => ([64, 32, 16, 256]) => ([64, 16, 8, 512]) => ([64, 8, 4, 1024])
            x = layer(x)
        x = self.classifier(x)
        return x

    def load_param(self, model_path, model):
